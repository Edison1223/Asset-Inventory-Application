...
class AssetScanner extends StatefulWidget {
  const AssetScanner({super.key});

  @override
  AssetScannerState createState() => AssetScannerState();
}

class AssetScannerState extends State<AssetScanner> with SingleTickerProviderStateMixin {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  List<Map<String, dynamic>> scannedAssets = [];
  List<Map<String, dynamic>> filteredAssets = [];
  Set<String> scannedRfids = {}; // Track scanned RFID IDs
  final Queue<String> _rfidQueue = Queue(); // Queue for RFID signals
  bool isScanning = false;
  String currentRfid = '';
  bool _isProcessing = false;
  bool isBuildingRfid = false; // Lockout for queuing RFID
  Timer? _debounceTimer;
  Timer? _buildTimeout; // Timeout for incomplete RFID
  final int _rfidLength = 24; // Fixed RFID length
  ...
  // Fetch asset details from Firebase using RFID ID
  ...
  void _processQueue() {
    if (_isProcessing || _rfidQueue.isEmpty) {
      ...
      setState(() {
        isBuildingRfid = false; // Allow queuing next RFID
      });
      return;
    }
    ...
    print("Processing RFID from queue: $rfid");
    fetchAssetDetails(rfid).then((_) {
      setState(() {
        _isProcessing = false;
        isBuildingRfid = false; // Allow queuing next RFID
      });
      _processQueue(); // Continue processing queue
    });
  }
  ...
  // Handle keyboard input using KeyEvent
  void _handleKeyPress(KeyEvent event) {
    if (!isScanning) {
      print("Input ignored: Scanning is stopped");
      return; // Explicitly block input when not scanning
    }

    if (event is KeyDownEvent) {
      final keyLabel = event.logicalKey.keyLabel;
      if (keyLabel.isNotEmpty && RegExp(r'^[0-9A-Z]$', caseSensitive: false).hasMatch(keyLabel)) {
        setState(() {
          currentRfid += keyLabel.toUpperCase();
          print("Building RFID: $currentRfid");
        });

        // Start timeout to reset if RFID isn't completed
        _buildTimeout?.cancel();
        ...
        if (currentRfid.length >= _rfidLength) {
          ...
          if (rfid.length == _rfidLength && rfid.startsWith('E')) {
            if (isBuildingRfid) {
              ...
              setState(() {
                currentRfid = ''; // Reset buffer
              });
              return;
            }
            print("Raw RFID input received: $rfid");
            if (!scannedRfids.contains(rfid) && !_rfidQueue.contains(rfid)) {
              setState(() {
                isBuildingRfid = true; // Lock queuing
              });
              _debounceTimer?.cancel();
              ...
              print("RFID already scanned or queued: $rfid");
              setState(() {
                isBuildingRfid = false; // Allow next RFID
              });
            }
          } else {
  ...
  // Custom dialog for showing messages
  ...
  // Show detailed asset information in a dialog
